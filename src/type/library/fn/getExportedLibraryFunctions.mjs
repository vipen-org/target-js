...
export default async function(context) {
	const entries = await scandir(
		path.join(context.root, "src", "export")
	)
	...
	for (const entry of ret) {
		const function_file_path = path.join(context.root, "src", "export", entry.canonical_path + ".mjs")
		const factory_file_path = path.join(context.root, "src", "export", entry.canonical_path + "Factory.mjs")
